...
const kafkaFunction = async () => {
  await admin.connect();
  const fetchTopicMetadataResponse = await admin.fetchTopicMetadata({
    topics: [TOPIC_NAME],
  });
  console.log("FETCH TOPICS METADETA RESPONSE:- ");
  console.dir(fetchTopicMetadataResponse, { depth: null, colors: true });
